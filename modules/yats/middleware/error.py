...
class ErrorCaptureMiddleware(object):
    ...
    def process_response(self, request, response):
        ...
        if sc in [404]:
            ...
            if not settings.DEBUG:
                ...
                yats_data = 'URL: %s\nRAW_POST: %s\nHOST_NAME: %s\nYATS_APP_VERSION: %s\nDJANGO_VERSION: %s\nPYTHON: %s' % (request.build_absolute_uri(), request.raw_post_data, gethostname(), get_version(), get_django_version(), get_python_version())
    ...
    def process_exception(self, request, exception):
        ...
        yats_data = 'TS: %s\nURL: %s\nRAW_POST: %s\nHOST_NAME: %s\nYATS_APP_VERSION: %s\nDJANGO_VERSION: %s\nPYTHON: %s' % (datetime.datetime.now(), request.build_absolute_uri(), request.raw_post_data, gethostname(), get_version(), get_django_version(), get_python_version())
